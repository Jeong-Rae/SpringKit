...
function remoteTask(id, sourceId, { name, status = "COMPLETE", parent = null, markdown = "", type = null, dependencies = [] } = {}) {
  const custom_fields = [{ id: "source-id", value: sourceId }];
  if (type !== null) custom_fields.push({ id: "source-type", value: type });
  return { id, name, status: { status }, parent, markdown_description: markdown, custom_fields, dependencies };
}
...
test("신규 Task와 Step을 parent 순서로 생성하고 source identity를 포함한다", async () => {
  ...
  assert.deepEqual(plan.operations.map((operation) => operation.type), ["CREATE_TASK", "CREATE_SUBTASK"]);
...
test("동일 remote state는 NO_CHANGE로 수렴한다", () => {
  const [task, step] = projectTaskCard(card);
  const remote = [
    remoteTask("r1", "sk-5", { name: task.displayTitle, markdown: task.markdown, type: "feature" }),
    remoteTask("r2", "sk-5-1", { name: step.displayTitle, markdown: step.markdown, parent: "r1" }),
  ];
...
test("title 변경은 CREATE가 아니라 UPDATE가 된다", () => {
  ...
  const remote = [remoteTask("r1", "sk-5", { name: "[sk-5] old", markdown: task.markdown, type: "feature" })];
...
test("managed dependency만 reconcile하고 unmanaged dependency는 보존한다", () => {
  ...
  const remoteDependency = remoteTask("r4", "sk-4", { name: dependency.displayTitle, markdown: dependency.markdown, type: "feature" });
  ...
  const plan = createSyncPlan([dependency, task], [remoteDependency, remoteCurrent, { id: "external", custom_fields: [] }], config);
  assert.equal(plan.operations.filter((operation) => operation.type === "ADD_DEPENDENCY").length, 1);
  assert.equal(plan.operations.filter((operation) => operation.type === "REMOVE_DEPENDENCY").length, 0);
});
