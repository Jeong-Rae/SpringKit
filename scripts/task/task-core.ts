...
function requireKeys(
	...
	if (actual.join("\0") !== keys.join("\0")) {
		errors.push(
			`${location}: 필드는 다음 순서와 정확히 일치해야 합니다: ${keys.join(", ")}`,
		);
	}
}
...
export function validateTaskCard(
	...
	const idValid =
		requireString(value.id, `${source}.id`, errors) &&
		taskIdPattern.test(value.id as string);
	...
	if (!taskTypes.includes(value.type as TaskType))
		errors.push(`${source}.type: 허용되지 않은 Task Type입니다.`);
	if (!statuses.includes(value.status as Status))
		errors.push(`${source}.status: 허용되지 않은 상태입니다.`);
	...
	if (requireStringArray(value.depends_on, `${source}.depends_on`, errors)) {
		...
		dependencies.forEach((dependency, index) => {
			if (!taskIdPattern.test(dependency)) {
				errors.push(
					`${source}.depends_on[${index}]: sk-<양의 정수> 형식이어야 합니다.`,
				);
			}
		});
		if (new Set(dependencies).size !== dependencies.length) {
			errors.push(
				`${source}.depends_on: 같은 Task ID를 중복하여 기록할 수 없습니다.`,
			);
		}
	}
	...
	if (
		(value.type === "feature" || value.type === "fix") &&
		Array.isArray(value.spec_refs) &&
		value.spec_refs.length === 0
	) {
		errors.push(
			`${source}.spec_refs: feature와 fix Task에는 한 개 이상의 SPEC 참조가 필요합니다.`,
		);
	}
	...
	if (!Array.isArray(value.steps) || value.steps.length === 0) {
		...
		value.steps.forEach((step, index) => {
			...
			if (expectedStepId && step.id !== expectedStepId) {
				errors.push(
					`${location}.id: 선언 순서에 따라 ${expectedStepId}이어야 합니다.`,
				);
			}
			requireString(step.title, `${location}.title`, errors);
			if (!statuses.includes(step.status as Status))
				errors.push(`${location}.status: 허용되지 않은 상태입니다.`);
			...
			if (
				index > 0 &&
				["running", "verifying", "done"].includes(step.status as string)
			) {
				const previous = value.steps[index - 1];
				if (!isRecord(previous) || previous.status !== "done") {
					errors.push(
						`${location}.status: 이전 Step이 done이어야 ${String(step.status)} 상태가 될 수 있습니다.`,
					);
				}
			}
		});
	...
	if (value.status === "done" && Array.isArray(value.steps)) {
		if (value.steps.some((step) => !isRecord(step) || step.status !== "done")) {
			errors.push(
				`${source}.status: Task가 done이면 모든 Step도 done이어야 합니다.`,
			);
		}
	}
	return errors;
...
export async function readTaskCard(
	...
	try {
		...
		throw new TaskToolError(
			`${file}: 파일을 읽을 수 없습니다. ${error instanceof Error ? error.message : String(error)}`,
		);
	}
	const document = parseDocument(text, { uniqueKeys: true });
	if (document.errors.length > 0) {
		throw new TaskToolError(
			document.errors.map((error) => `${file}: ${error.message}`).join("\n"),
		);
	}
	...
	if (
		...
		if (basename(file) !== expected)
			errors.push(`${file}: 파일명은 ${expected}이어야 합니다.`);
	}
	if (
		requireCanonicalFormat &&
		errors.length === 0 &&
		text !== serializeTaskCard(value as TaskCard)
	) {
		errors.push(
			`${file}: canonical YAML 형식이 아닙니다. add-task.ts가 생성하는 형식을 사용해야 합니다.`,
		);
	}
...
export async function loadTaskCards(
	tasksDir: string,
): Promise<Map<string, { card: TaskCard; file: string }>> {
	let names: string[];
	try {
		...
		throw new TaskToolError(
			`${tasksDir}: Task 디렉터리를 읽을 수 없습니다. ${error instanceof Error ? error.message : String(error)}`,
		);
	}
	...
	for (const name of names) {
		...
		try {
			...
			if (cards.has(card.id))
				errors.push(`${file}: 중복 Task ID입니다: ${card.id}`);
...
function reachable(
	from: string,
	target: string,
	cards: Map<string, { card: TaskCard }>,
	...
	return entry
		? entry.card.depends_on.some((dependency) =>
				reachable(dependency, target, cards, visited),
			)
		: false;
}
...
export function validateTaskGraph(
	...
	for (const [id, { card }] of cards) {
		...
		for (const dependency of card.depends_on) {
			if (dependency === id)
				errors.push(`${id}: 자기 자신을 dependency로 사용할 수 없습니다.`);
			if (seen.has(dependency))
				errors.push(`${id}: dependency가 중복되었습니다: ${dependency}`);
			seen.add(dependency);
			if (!cards.has(dependency))
				errors.push(`${id}: 선행 Task를 찾을 수 없습니다: ${dependency}`);
		}
	}
	...
	const visit = (id: string, path: string[]): void => {
		if (visiting.has(id)) {
			...
			errors.push(
				`Task DAG에 cycle이 있습니다: ${[...path.slice(start), id].join(" -> ")}`,
			);
			return;
		}
	...
	if (!errors.some((error) => error.includes("cycle"))) {
		for (const [id, { card }] of cards) {
			for (const dependency of card.depends_on) {
				const otherDependencies = card.depends_on.filter(
					(candidate) => candidate !== dependency,
				);
				if (
					otherDependencies.some((other) => reachable(other, dependency, cards))
				) {
					errors.push(
						`${id}: ${dependency}는 전이적으로 충족되므로 직접 dependency에서 제거해야 합니다.`,
					);
				}
			}
		}
...
export async function writeTaskCardAtomically(
	file: string,
	card: TaskCard,
): Promise<void> {
	...
	try {
		...
		throw new TaskToolError(
			`${file}: Task Card를 기록할 수 없습니다. ${error instanceof Error ? error.message : String(error)}`,
		);
	}
}

export async function createTaskCardAtomically(
	file: string,
	card: TaskCard,
): Promise<void> {
	const temporary = `${file}.${process.pid}.${Date.now()}.tmp`;
	try {
		...
		await unlink(temporary).catch(() => undefined);
		throw new TaskToolError(
			`${file}: 새 Task Card를 기록할 수 없습니다. ${error instanceof Error ? error.message : String(error)}`,
		);
	}
}

export function extractTasksDir(args: string[]): {
	tasksDir: string;
	rest: string[];
} {
	const rest: string[] = [];
	...
	for (let index = 0; index < args.length; index += 1) {
		if (args[index] === "--tasks-dir") {
			...
			if (!value)
				throw new TaskToolError("--tasks-dir 뒤에 경로가 필요합니다.", 2);
