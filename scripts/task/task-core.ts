...
function requireString(value: unknown, location: string, errors: string[]): value is string {
...
export function validateTaskCard(value: unknown, source = "Task Card"): string[] {
  ...
  if (
    ...
    errors.push(
      `${source}.requirements: feature와 fix Task에는 한 개 이상의 요구 사항이 필요합니다.`,
    );
  }
  ...
  if (!Array.isArray(steps) || steps.length === 0) {
    ...
    steps.forEach((step, index) => {
      ...
      const expectedStepId = idValid ? `${id}-${index + 1}` : undefined;
  ...
  if (
    value.status === "done" &&
    Array.isArray(steps) &&
    steps.some((step) => !isRecord(step) || step.status !== "done")
  ) {
...
export async function readTaskCard(
  ...
  if (extname(file) !== ".yaml")
    throw new TaskToolError(`${file}: Task Card 확장자는 .yaml이어야 합니다.`);
  ...
  if (
    ...
    errors.push(
      `${file}: canonical YAML 형식이 아닙니다. add-task.mjs가 생성하는 형식을 사용해야 합니다.`,
    );
  }
...
function reachable(
  ...
  return entry
    ? entry.card.depends_on.some((dependency) => reachable(dependency, target, cards, visited))
    : false;
}
...
export function validateTaskGraph(cards: Map<string, { card: TaskCard }>): string[] {
  ...
  if (!errors.some((error) => error.includes("cycle"))) {
    for (const [id, { card }] of cards) {
      for (const dependency of card.depends_on) {
        const others = card.depends_on.filter((candidate) => candidate !== dependency);
        if (others.some((other) => reachable(other, dependency, cards))) {
          errors.push(
