...
function requireValidGraph(cards: Awaited<ReturnType<typeof loadTaskCards>>): void {
...
runCli(async () => {
  ...
  if (command === "register") {
    const source = rest[1];
    if (!source) {
      throw new TaskToolError("사용법: node ./scripts/task/add-task.ts register <card-file> [--depends-on <task-id>...] [--tasks-dir <path>]", 2);
    }
  ...
  if (command === "dependency") {
    if (rest.length !== 3) {
      throw new TaskToolError("사용법: node ./scripts/task/add-task.ts dependency <task-id> <dependency-id> [--tasks-dir <path>]", 2);
    }
    const taskId = rest[1];
    ...
    const updated = { ...entry.card, depends_on: [...entry.card.depends_on, dependencyId] };
    const candidate = new Map(cards);
  ...
  throw new TaskToolError("사용법: node ./scripts/task/add-task.ts <register|dependency> ...", 2);
});
