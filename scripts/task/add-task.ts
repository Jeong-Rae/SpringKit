...
runCli(async () => {
  ...
  if (command === "register") {
    ...
    if (!source) {
      throw new TaskToolError(
        "사용법: node <skill>/scripts/add-task.mjs register <card-file> [--depends-on <task-id>...] [--tasks-dir <path>]",
        2,
      );
    }
  ...
  if (command === "dependency") {
    if (rest.length !== 3) {
      throw new TaskToolError(
        "사용법: node <skill>/scripts/add-task.mjs dependency <task-id> <dependency-id> [--tasks-dir <path>]",
        2,
      );
    }
    const taskId = rest[1];
