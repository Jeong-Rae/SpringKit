import { extractTasksDir, loadTaskCards, runCli, TaskToolError, validateTaskGraph } from "./task-core.ts";

runCli(async () => {
  const { tasksDir, rest } = extractTasksDir(process.argv.slice(2));
  if (rest.length !== 0) {
    throw new TaskToolError("사용법: node ./scripts/task/validate-tasks.ts [--tasks-dir <path>]", 2);
  }
