...
test("유효한 Task Card를 승인한다", async ()=>{
    ...
    const result = await runScript("lint-task.mjs", [
        join(tasksDir, "sk-5.json")
    ]);
...
test("필수 검증과 Step 순서 위반을 함께 보고한다", async ()=>{
    ...
    const { tasksDir } = await temporaryTasks();
    const file = join(tasksDir, "sk-5.json");
...
test("canonical JSON 형식이 아니면 거부한다", async ()=>{
    const { tasksDir } = await temporaryTasks();
    const file = join(tasksDir, "sk-5.json");
    const card = validCard("sk-5");
    await writeCard(file, card);
    ...
    await writeFile(file, `${JSON.stringify(canonical, null, 4)}\n`);
    const result = await runScript("lint-task.mjs", [
        file
    ]);
    assert.equal(result.code, 1);
