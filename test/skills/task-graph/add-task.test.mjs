...
test("Card를 canonical 이름으로 등록하고 lint할 수 있다", async ()=>{
    ...
    const added = await runScript("add-task.mjs", [
        "register",
        draft,
        "--depends-on",
        "sk-1",
    ...
    const linted = await runScript("lint-task.mjs", [
        join(tasksDir, "sk-2.json")
    ]);
    assert.equal(linted.code, 0, linted.stderr);
    const registered = JSON.parse(await readFile(join(tasksDir, "sk-2.json"), "utf8"));
    assert.deepEqual(registered.depends_on, ["sk-1"]);
});
test("등록 충돌은 기존 Card를 변경하지 않는다", async ()=>{
    const { root, tasksDir } = await temporaryTasks([
        validCard("sk-1")
    ]);
    const existing = join(tasksDir, "sk-1.json");
    ...
    const result = await runScript("add-task.mjs", [
        "register",
        draft,
        "--tasks-dir",
        tasksDir
...
test("기존 Task에 dependency를 추가하고 재실행해도 결과가 같다", async ()=>{
    ...
    const first = await runScript("add-task.mjs", args);
    assert.equal(first.code, 0, first.stderr);
    const file = join(tasksDir, "sk-2.json");
    const once = await readFile(file, "utf8");
    const second = await runScript("add-task.mjs", args);
...
test("cycle을 만드는 dependency는 원본을 변경하지 않는다", async ()=>{
    ...
    const file = join(tasksDir, "sk-1.json");
...
test("전이 dependency를 추가하면 원본을 변경하지 않는다", async ()=>{
    ...
    const file = join(tasksDir, "sk-3.json");
    const before = await readFile(file, "utf8");
    const result = await runScript("add-task.mjs", [
        "dependency",
        "sk-3",
