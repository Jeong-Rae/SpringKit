...
test("빈 Task 디렉터리에서 sk-1을 발급한다", async () => {
  ...
  const result = await runScript("issue-task-id.ts", ["--tasks-dir", tasksDir]);
  assert.equal(result.code, 0);
  assert.equal(result.stdout, "sk-1\n");
});

test("순번의 공백을 채우지 않고 최대 순번 다음 ID를 발급한다", async () => {
  const { tasksDir } = await temporaryTasks([validCard("sk-1"), validCard("sk-3"), validCard("sk-5")]);
  const result = await runScript("issue-task-id.ts", ["--tasks-dir", tasksDir]);
  assert.equal(result.code, 0);
...
test("잘못된 Card가 있으면 ID를 발급하지 않는다", async () => {
  ...
  await writeCard(join(tasksDir, "sk-2.yaml"), invalid);
  const result = await runScript("issue-task-id.ts", ["--tasks-dir", tasksDir]);
...
test("알 수 없는 인자는 종료 코드 2로 거부한다", async () => {
  const { tasksDir } = await temporaryTasks();
  const result = await runScript("issue-task-id.ts", ["--tasks-dir", tasksDir, "extra"]);
