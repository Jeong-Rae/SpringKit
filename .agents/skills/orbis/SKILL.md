...
description: >-
  ...
  조율합니다. Worker는 구현 전에 필요한 질문으로 책임과 경계를 확인하고
  승인된 범위에서 구현과 검증을 수행합니다. 오케스트레이터는 실제 저장소
  상태를 검증하고 논리적 커밋이 완결될 때까지 후속 작업을 조정합니다.
  하나의 코딩 요청을 여러 Worker 책임으로 나누어 한 논리적 커밋으로
  완결해야 할 때 사용합니다.
---
...
`task_name`은 오케스트레이터가 작업을 구분하는 내부 이름입니다. Orbis의 Worker 요청, 질의, 보고에는 task 관리 정보를 넣지 않습니다.

`spawn_agent(...)`가 반환한 `agent_id`를 Worker ID로 기록합니다. 이후 Worker 응답과 후속 호출은 이 Worker ID로 연결합니다.
...
5. 반환된 `agent_id`를 Worker ID로 기록하고 내부 task 관리 정보와 연결합니다.
6. Worker는 저장소와 준비 자료를 읽고 스스로 확정하기 어려운 사항이 있으면 Worker 질의 스키마에 맞춰 질문합니다.
7. 오케스트레이터는 질문에 답하고 잘못된 이해를 고치며 필요한 정보와 판단을 제공합니다.
8. Worker는 질문이 남아 있으면 다시 `QUESTION`을 반환하고, 모두 해소되면 `READY`를 반환합니다.
...
- task 이름과 Worker ID의 매핑은 오케스트레이터가 관리합니다.
