...
소유권에 포함하지 않았거나 보고서에 적지 않은 파일이 바뀌었으면 이유를 확인합니다. 구현이 끝났다는 이유로 소유권을 벗어난 변경을 사후 승인하지 않습니다.

## 검증

Worker가 보고한 검증이 해당 책임이 요구하는 동작을 증명하는지 확인합니다. 근거가 부족하면 필요한 검증을 추가로 요청합니다.
