...
보고는 간결하게 작성합니다. 각 자유 서술 항목은 3줄 이내, 전체 보고 내용은 약 10줄을 권장합니다.

이 분량은 형식 검증을 위한 엄격한 제한이 아니라 장황한 응답을 줄이기 위한 가이드입니다. 필요한 검증 결과, 이슈, 변경 경로는 줄 수를 맞추려고 생략하지 않습니다. `STATUS`와 `FILES`의 경로 목록은 이 분량 목표에서 제외합니다.
