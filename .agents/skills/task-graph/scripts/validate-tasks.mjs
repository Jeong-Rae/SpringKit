import { extractTasksDir, loadTaskCards, runCli, TaskToolError, validateTaskGraph } from "./task-core.mjs";
runCli(async ()=>{
    const { tasksDir, rest } = extractTasksDir(process.argv.slice(2));
    if (rest.length !== 0) {
        throw new TaskToolError("사용법: node <skill>/scripts/validate-tasks.mjs [--tasks-dir <path>]", 2);
    }
