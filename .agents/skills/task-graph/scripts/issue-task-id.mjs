...
//#endregion
//#region scripts/task/issue-task-id.ts
runCli(async () => {
	const { tasksDir, rest } = extractTasksDir(process.argv.slice(2));
	if (rest.length !== 0) throw new TaskToolError("사용법: node <skill>/scripts/issue-task-id.mjs [--tasks-dir <path>]", 2);
	const maximum = [...(await loadTaskCards(tasksDir)).keys()].reduce((current, id) => {
