...
import { createTaskCardAtomically, extractTasksDir, loadTaskCards, readTaskCard, runCli, TaskToolError, validateTaskCard, validateTaskGraph, writeTaskCardAtomically } from "./task-core.mjs";
...
runCli(async ()=>{
    ...
    if (command === "register") {
        const source = rest[1];
        if (!source) {
            throw new TaskToolError("사용법: node <skill>/scripts/add-task.mjs register <card-file> [--depends-on <task-id>...] [--tasks-dir <path>]", 2);
        }
        const dependencies = [];
        ...
        candidate.set(card.id, {
            card,
            file: join(tasksDir, `${card.id}.json`)
        });
        requireValidGraph(candidate);
        await createTaskCardAtomically(join(tasksDir, `${card.id}.json`), card);
        console.log(`${card.id}: Task를 등록했습니다.`);
        return;
    }
    if (command === "dependency") {
        if (rest.length !== 3) {
            throw new TaskToolError("사용법: node <skill>/scripts/add-task.mjs dependency <task-id> <dependency-id> [--tasks-dir <path>]", 2);
        }
        const taskId = rest[1];
