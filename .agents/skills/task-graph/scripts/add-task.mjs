...
//#endregion
//#region scripts/task/task-core.ts
...
var TaskToolError = class extends Error {
	exitCode;
...
function requireKeys(value, keys, location, errors) {
	if (Object.keys(value).join("\0") !== keys.join("\0")) errors.push(`${location}: 필드는 다음 순서와 정확히 일치해야 합니다: ${keys.join(", ")}`);
}
...
function requireStringArray(value, location, errors, requireItem = false) {
	...
	if (requireItem && value.length === 0) errors.push(`${location}: 한 개 이상의 항목이 필요합니다.`);
	value.forEach((item, index) => requireString(item, `${location}[${index}]`, errors));
	return value.every((item) => typeof item === "string" && item.trim() !== "");
}
function validateTaskCard(value, source = "Task Card") {
	const errors = [];
	if (!isRecord(value)) return [`${source}: YAML 최상위 값은 객체여야 합니다.`];
	requireKeys(value, taskKeys, source, errors);
	const id = value.id;
	const idValid = requireString(id, `${source}.id`, errors) && taskIdPattern.test(id);
	if (typeof value.id === "string" && !taskIdPattern.test(value.id)) errors.push(`${source}.id: sk-<양의 정수> 형식이어야 합니다.`);
	...
	if (requireStringArray(value.depends_on, `${source}.depends_on`, errors)) {
		value.depends_on.forEach((dependency, index) => {
			if (!taskIdPattern.test(dependency)) errors.push(`${source}.depends_on[${index}]: sk-<양의 정수> 형식이어야 합니다.`);
		});
		if (new Set(value.depends_on).size !== value.depends_on.length) errors.push(`${source}.depends_on: 같은 Task ID를 중복하여 기록할 수 없습니다.`);
	}
	requireStringArray(value.requirements, `${source}.requirements`, errors);
	if ((value.type === "feature" || value.type === "fix") && Array.isArray(value.requirements) && value.requirements.length === 0) errors.push(`${source}.requirements: feature와 fix Task에는 한 개 이상의 요구 사항이 필요합니다.`);
	if (!isRecord(value.scope)) errors.push(`${source}.scope: in과 out을 가진 객체여야 합니다.`);
	else {
		requireKeys(value.scope, ["in", "out"], `${source}.scope`, errors);
	...
	if (!Array.isArray(steps) || steps.length === 0) errors.push(`${source}.steps: 한 개 이상의 Step이 필요합니다.`);
		...
		const expectedStepId = idValid ? `${id}-${index + 1}` : void 0;
		requireString(step.id, `${location}.id`, errors);
		if (expectedStepId && step.id !== expectedStepId) errors.push(`${location}.id: 선언 순서에 따라 ${expectedStepId}이어야 합니다.`);
		...
		if (index > 0 && [
			...
			if (!isRecord(previous) || previous.status !== "done") errors.push(`${location}.status: 이전 Step이 done이어야 ${String(step.status)} 상태가 될 수 있습니다.`);
		}
	});
	if (value.status === "done" && Array.isArray(steps) && steps.some((step) => !isRecord(step) || step.status !== "done")) errors.push(`${source}.status: Task가 done이면 모든 Step도 done이어야 합니다.`);
	return errors;
}
async function readTaskCard(file, requireMatchingFilename = false, requireCanonicalFormat = false) {
	if (extname(file) !== ".yaml") throw new TaskToolError(`${file}: Task Card 확장자는 .yaml이어야 합니다.`);
	let text;
	try {
	...
	const document = (0, import_dist.parseDocument)(text, { uniqueKeys: true });
	if (document.errors.length > 0) throw new TaskToolError(document.errors.map((error) => `${file}: ${error.message}`).join("\n"));
	...
	if (requireCanonicalFormat && errors.length === 0 && text !== serializeTaskCard(value)) errors.push(`${file}: canonical YAML 형식이 아닙니다. add-task.mjs가 생성하는 형식을 사용해야 합니다.`);
	if (errors.length > 0) throw new TaskToolError(errors.join("\n"));
	return value;
}
async function loadTaskCards(tasksDir) {
	let names;
	try {
...
function reachable(from, target, cards, visited = /* @__PURE__ */ new Set()) {
...
function validateTaskGraph(cards) {
	const errors = [];
	...
	const visit = (id, path) => {
		...
		visiting.add(id);
		cards.get(id)?.card.depends_on.forEach((dependency) => {
	...
	if (!errors.some((error) => error.includes("cycle"))) {
		for (const [id, { card }] of cards) for (const dependency of card.depends_on) if (card.depends_on.filter((candidate) => candidate !== dependency).some((other) => reachable(other, dependency, cards))) errors.push(`${id}: ${dependency}는 전이적으로 충족되므로 직접 dependency에서 제거해야 합니다.`);
	}
	return [...new Set(errors)];
}
function serializeTaskCard(card) {
	return (0, import_dist.stringify)(card, {
		indent: 2,
		lineWidth: 0
	});
}
async function writeTaskCardAtomically(file, card) {
...
async function createTaskCardAtomically(file, card) {
	const temporary = `${file}.${process.pid}.${Date.now()}.tmp`;
	try {
		await writeFile(temporary, serializeTaskCard(card), {
			encoding: "utf8",
...
function extractTasksDir(args) {
	const rest = [];
	let tasksDir = resolve(process.cwd(), "tasks");
	for (let index = 0; index < args.length; index += 1) if (args[index] === "--tasks-dir") {
...
//#endregion
//#region scripts/task/add-task.ts
function requireValidGraph(cards) {
...
runCli(async () => {
	...
	if (command === "register") {
		const source = rest[1];
		if (!source) throw new TaskToolError("사용법: node <skill>/scripts/add-task.mjs register <card-file> [--depends-on <task-id>...] [--tasks-dir <path>]", 2);
		const dependencies = [];
		for (let index = 2; index < rest.length; index += 1) {
			if (rest[index] !== "--depends-on" || !rest[index + 1]) throw new TaskToolError("--depends-on 뒤에 Task ID가 필요합니다.", 2);
			dependencies.push(rest[index + 1]);
			index += 1;
		}
		const card = await readTaskCard(resolve(source));
		if (cards.has(card.id)) throw new TaskToolError(`${card.id}: 이미 등록된 Task입니다.`);
		...
		candidate.set(card.id, {
			card,
			file: join(tasksDir, `${card.id}.yaml`)
		});
	...
	if (command === "dependency") {
		if (rest.length !== 3) throw new TaskToolError("사용법: node <skill>/scripts/add-task.mjs dependency <task-id> <dependency-id> [--tasks-dir <path>]", 2);
		const taskId = rest[1];
	...
	throw new TaskToolError("사용법: node <skill>/scripts/add-task.mjs <register|dependency> ...", 2);
});
